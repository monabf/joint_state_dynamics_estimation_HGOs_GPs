...
class LogGaussian(Gaussian):
    ...
    def lnpdf(self, x):
        return self.constant - 0.5 * np.square(
            np.log(x) - self.mu) / self.sigma2 - np.log(x)
...
class Gamma(Prior):
    ...
    def summary(self):
        ret = {"E[x]": self.a / self.b, \
               ...
               "Entropy": gammaln(self.a) - (self.a - 1.) * digamma(
                   self.a) - np.log(self.b) + self.a}
        if self.a > 1:
...
class DGPLVM_KFDA(Prior):
    ...
    def lnpdf(self, x):
        ...
        J = a_trans.dot(K).dot(self.a) - a_trans.dot(K).dot(self.A).dot(
            inv_part).dot(self.A).dot(K).dot(self.a)
...
class DGPLVM(Prior):
        ...
        for k in range(self.datanum):
            for i in data_idx:
                ...
                if k in lst_idx_all[i]:
                    ...
                    Sig_beta_B_i_all[k] += float(N_i) / self.datanum * (
                            beta * B_i[i])
                else:
    ...
    def lnpdf_grad(self, x):
        ...
        Sig_beta_B_i_all = self.compute_sig_beta_Bi(data_idx, M_i, M_0,
                                                    lst_idx_all)
        ...
        # Calculating DJ/DXk
        DJ_Dxk = 2 * (
                Sb_inv_N_trans_minus.dot(Sw_trans).dot(Sb_inv_N_trans).dot(
                    Sig_beta_B_i_all) + Sb_inv_N_trans.dot(
            Sig_alpha_W_i))
...
class HalfT(Prior):
    ...
    def __init__(self, A, nu):
        self.A = float(A)
        self.nu = float(nu)
        self.constant = gammaln(.5 * (self.nu + 1.)) - gammaln(
            .5 * self.nu) - .5 * np.log(np.pi * self.A * self.nu)

    def __str__(self):
        return "hT({:.2g}, {:.2g})".format(self.A, self.nu)

    def lnpdf(self, theta):
        return (theta > 0) * (self.constant - .5 * (self.nu + 1) * np.log(
            1. + (1. / self.nu) * (theta / self.A) ** 2))
    ...
    def lnpdf_grad(self, theta):
        ...
        v = self.nu
        sigma2 = self.A
        grad[above_zero] = -0.5 * (v + 1) * (2 * theta[above_zero]) / (
